"""
EE627A Final Project - PySpark Matrix Factorization (Yahoo Music)
Jude Eschete

Lineage:
  midterm v3 heuristic (Kaggle 0.871)
    -> HW8 Part 1 tiebreaker hybrid + PySpark ALS (Kaggle 0.875)
    -> Final project (this file)

v2 formula that v3 builds on:
    has_album*album + has_artist*artist + genre_count*genre_mean/10

Concepts carried forward from HW7 (MATLAB SGD matrix factorization):
  - Matrix factorization decomposes R ~ P * Q^T with K latent factors.
    HW7 implemented SGD MF by hand in MATLAB (alpha, beta, K, steps).
    This file uses PySpark ALS (rank, maxIter, regParam) -- same
...
Hybrid strategy:
  - Reuse v3's parsers, enrichment, validation framework.
  - Add an ALS-trained latent score as a NEW feature per (user, track).
  - score = v2_score + epsilon * mf_score   (epsilon = 1e-3)
    MF acts as a tiebreaker ONLY when v2 produces exact ties (users with
    zero album/artist/genre overlap). The v2 magnitudes dominate every
    other case by ~1000x, so the Kaggle floor is guaranteed to be 0.871.
  - Week 6 testTrack_hierarchy.txt is wired in as a per-test-row lineage
    override so compute_features can use the correct album/artist/genre
    even when trackData2.txt is missing entries.
...
def run_pyspark_als(rank=ALS_RANK, max_iter=ALS_MAX_ITER,
    ...
    pure_mf_path = os.path.join(OUTPUT_DIR, "submission_final_pyspark_mf.csv")
...
RESULTS_PATH = os.path.join(
    RESULTS_DIR,
    f"final_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
)
...
def main():
    ...
    print("=" * 60)
    print("  EE627A Final Project - PySpark Matrix Factorization")
    print("  Jude Eschete")
    print("=" * 60)
    ...
    # ---- PySpark ALS training (Final Project core) ----
    # Run ALS on the Week-11 pre-flattened CSVs using the sample code
    # pattern. Also writes submission_final_pyspark_mf.csv (pure MF).
    ...
    run_strategy(
        ...
        strategy_v2_mf_tiebreaker,
        os.path.join(OUTPUT_DIR, "submission_final_hybrid.csv"),
    ...
    run_strategy(
        user_ratings, enriched_meta, test_users,
        strategy_v2_mf_gated,
        os.path.join(OUTPUT_DIR, "submission_final_hybrid_gated.csv"),
    ...
    print()
    print("  Files to upload for Final Project:")
    print(f"    submission_final_pyspark_mf.csv       <-- pure PySpark MF")
    print(f"    submission_final_hybrid.csv           <-- v2 + MF tiebreaker")
    print(f"    submission_final_hybrid_gated.csv     <-- v2 + MF evidence-gated")
...
def _log_crash(exc):
    ...
    err_path = os.path.join(
        RESULTS_DIR,
        f"final_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
    )
