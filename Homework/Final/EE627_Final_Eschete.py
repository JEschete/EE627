...
LGBM_CACHE       = os.path.join(CACHE_DIR, "lgbm_lambdarank.txt")
...
# Item-item CF: original config (already cached) + two tuning variants.
# v5 stacks all three as separate features so the meta-learner can blend.
IICF_VARIANTS    = [
    ("iicf_a", 30,  5),   # original v4 config (cached as iicf_k30_m5.npz)
    ("iicf_b", 50,  3),   # more neighbors, looser similarity threshold
    ("iicf_c", 20, 10),   # fewer neighbors, stricter threshold
]
...
IICF_CACHE       = os.path.join(CACHE_DIR,
                                  f"iicf_k{IICF_TOPK}_m{IICF_MIN_CO}.npz")
...
def build_item_inverted_index(user_ratings):
...
def compute_or_load_iicf(test_users, user_ratings,
                           topk=None, min_co=None,
                           item_index_cache=None):
    """Compute item-item CF scores for all 120K test pairs, with caching.

    Args:
      topk, min_co       : hyperparameters for this variant.  Default
                            falls back to the module-level IICF_TOPK /
                            IICF_MIN_CO (variant 'a').
      item_index_cache   : optional pre-built (item_users, item_norms)
                            tuple, so multiple variants share the index
                            (it costs ~5s and ~1GB to build).
    """
    if topk is None:
        topk = IICF_TOPK
    if min_co is None:
    ...
    cache_path = os.path.join(CACHE_DIR, f"iicf_k{topk}_m{min_co}.npz")
    ...
    if item_index_cache is None:
        print(f"  Building item inverted index ...")
        t0 = time.time()
        item_users, item_norms = build_item_inverted_index(user_ratings)
...
def main():
    ...
    # 10. Item-item CF tuning variants for v5
    section("ITEM-ITEM CF TUNING VARIANTS (for v5)")
    print("  Building shared item index once for all variants ...")
    t_idx = time.time()
    iicf_index = build_item_inverted_index(user_ratings)
    print(f"  {len(iicf_index[0]):,} items indexed  [{elapsed(t_idx)}]")
    iicf_variants_dict = {}
    for name, k, m in IICF_VARIANTS:
        print(f"\n  Variant {name}: topk={k}, min_co={m}")
        iicf_variants_dict[name] = compute_or_load_iicf(
            test_users, user_ratings,
            topk=k, min_co=m, item_index_cache=iicf_index)

    # 11. Stacked meta-learner v5 -- iicf variants stacked, ALS dropped.
    ...
    if stacked_v5_path is not None:
        print(f"    submission_final_stacked_v5_real.csv     stacked v5 (3 iicf variants, no ALS)")
    print()
    ...
    if stacked_v5_path is not None:
        print(f"    submission_final_stacked_v5_real.csv    "
              f"(v5 meta-learner -- 3 iicf variants, no ALS)")
    print()
