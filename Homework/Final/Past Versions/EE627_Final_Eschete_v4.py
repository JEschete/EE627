...
ALS_RANK         = 80      # bumped from 20 -- 0.621 was too coarse
ALS_REG          = 0.05
ALS_ITERS        = 20
ALS_CACHE        = os.path.join(CACHE_DIR,
                                  f"als_r{ALS_RANK}_i{ALS_ITERS}.npz")
IICF_TOPK        = 30      # how many of user's history items to score against
IICF_MIN_CO      = 5       # min co-raters to count as a similarity
...
def compute_or_load_iicf(test_users, user_ratings):
    ...
    if os.path.exists(IICF_CACHE):
        print(f"  Loading cached item-item CF scores from "
              f"{os.path.basename(IICF_CACHE)} ...")
        z = np.load(IICF_CACHE, allow_pickle=True)
        keys   = z["keys"]
        values = z["values"]
    ...
    print(f"  Scoring item-item CF (top-{IICF_TOPK} history, "
          f"min_co={IICF_MIN_CO}) ...")
    ...
    np.savez(IICF_CACHE, keys=keys_arr, values=values_arr)
    print(f"  Cached -> {os.path.basename(IICF_CACHE)}")
    return out
...
def main():
    ...
    if stacked_v4_path is not None:
        print(f"    submission_final_stacked_v4.csv          stacked v4 (item-item CF + v3)")
        print(f"    submission_final_stacked_v4_iicf_only.csv  pure item-item CF top-3")
    print()
    ...
    if stacked_v4_path is not None:
        print(f"    submission_final_stacked_v4.csv         "
              f"(v4 meta-learner -- adds item-item CF)")
    print()
